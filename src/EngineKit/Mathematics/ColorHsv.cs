...
namespace EngineKit.Mathematics
{
    ...
    public struct ColorHsv : IEquatable<ColorHsv>, IFormattable
    {
        ...
        {
            float max = MathF.Max(color.Red, MathF.Max(color.Green, color.Blue));
            float min = MathF.Min(color.Red, MathF.Min(color.Green, color.Blue));
            ...
            if (delta > 0.0f)
            {
                if (color.Red >= max)
                    ...
                    h = (color.Blue - color.Red) / delta + 2.0f;
                else
                    h = (color.Red - color.Green) / delta + 4.0f;
                h *= 60.0f;
            ...
            return new ColorHsv(h, s, max, color.Alpha);
        }
